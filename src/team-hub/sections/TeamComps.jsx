...
function CompCard({ comp, onDelete, onSave }) {
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState(false);
  const [form, setForm] = useState({ name: comp.name, slots: comp.slots, strengths: comp.strengths, weaknesses: comp.weaknesses, counter: comp.counter, notes: comp.notes });

  const setSlot = (i, val) => setForm(f => { const s = [...f.slots]; s[i] = val; return { ...f, slots: s }; });

  const commit = () => { onSave({ ...comp, ...form }); setEditing(false); };
      ...
      {editing ? (
        ...
        <>
          <div className="card-header">
            ...
            <div className="flex gap-8">
              <button className="btn-link" onClick={() => { setEditing(true); setOpen(true); }}>[edit]</button>
              <button className="btn-link" onClick={onDelete}>[delete]</button>
            </div>
          </div>
          {open && (
            <div style={{ marginTop: 10 }}>
              {comp.strengths && <div className="mb-8"><div className="text-muted" style={{ marginBottom: 3 }}>STRENGTHS</div><div style={{ fontSize: 14, whiteSpace: 'pre-wrap' }}>{comp.strengths}</div></div>}
              {comp.weaknesses && <div className="mb-8"><div className="text-muted" style={{ marginBottom: 3 }}>WEAKNESSES</div><div style={{ fontSize: 14, whiteSpace: 'pre-wrap' }}>{comp.weaknesses}</div></div>}
              {comp.counter && <div className="mb-8"><div className="text-muted" style={{ marginBottom: 3 }}>HOW TO PLAY AGAINST</div><div style={{ fontSize: 14, whiteSpace: 'pre-wrap' }}>{comp.counter}</div></div>}
              {comp.notes && <div><div className="text-muted" style={{ marginBottom: 3 }}>NOTES</div><div style={{ fontSize: 14, whiteSpace: 'pre-wrap' }}>{comp.notes}</div></div>}
            </div>
          )}
        </>
      )}
...
export default function TeamComps() {
  ...
  const filtered = comps.filter(c => filter.length === 0 || filter.every(cls => c.slots.includes(cls)));
  ...
  return (
    <div>
      ...
      <div className="filter-toggles">
        ...
        {filter.length > 0 && <button className="btn-link" style={{ fontSize: 11 }} onClick={() => setFilter([])}>clear filter</button>}
      </div>
      <div className="section-actions">
        <button className="btn btn-sm" onClick={() => setShowForm(s => !s)}>{showForm ? '− Cancel' : '+ Add Comp'}</button>
      </div>
      {showForm && (
        <div className="inline-form mb-12">
          <div className="field mb-8"><label>Comp Name</label><input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Triple Heavy" /></div>
          <div className="field-row mb-8">
            {[0, 1, 2].map(i => (
              <div className="field" key={i}>
                <label>Slot {i + 1}</label>
      ...
      {filtered.map(c => (
        <CompCard
          key={c.id}
          comp={c}
          onDelete={() => save(comps.filter(x => x.id !== c.id))}
          onSave={(updated) => save(comps.map(x => x.id === updated.id ? updated : x))}
        />
      ))}
    </div>
  );
